...
1. a=[]とする。
2. lをgの長さとする。
3. for i in 1 to g, repeat this process:
    1. cをgのi文字目とする。
    2. もしcが「ギ」であれば、aに(i,空文字列)を追加する。
    3. もしcが「グ」または「ゲ」または「ゴ」であれば、aの末尾の要素の第2要素の末尾にcを追加する。
...
2. Gの末尾によって分岐する。
    ...
    2. そうでないとき
        1. Gの部分ギアルをPとする。
        2. Pの最後の要素の第2要素が「グアル」のとき
            1. 値rを以下を満たす最大のrとして定める。
                1. Pの最後の要素の第2要素をpとする。
                2. Pの長さをlとする。
                3. このとき、P[r]の第2要素<pであり、かつ任意のr<r'<lに対してP[r']の第2要素>=pである。
            2. もしそのようなrが存在すれば、
                1. gをGの左端からGの「「P[r]の第1要素-1」文字目」までの部分とする。
                2. bをGの「「P[r]の第1要素」文字目」からGの「右端から4文字目」までの部分に「ギ」を足した文字列とする。
                3. G'=g+b+b+b+bとする。
            3. もしそのようなrが存在しなければ、
                1. gを空文字列とする。
